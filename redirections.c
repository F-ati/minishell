...
/*   Updated: 2024/11/10 15:00:01 by fel-aziz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while(save_list != NULL)
	{
		...
		while(save_redir != NULL)
		{
			if(save_redir->type == IN)
			{
				...
				save_list->fd_input = ft_open_input(save_redir->file_name);
			}
			if(save_redir->type == OUT)
			{
				close(save_list->fd_output);
				save_list->fd_output = ft_open_output(save_redir->file_name);
			}
			if(save_redir->type == APPEND)
			{
				close(save_list->fd_output);
				save_list->fd_output = ft_open_append(save_redir->file_name);
			}
